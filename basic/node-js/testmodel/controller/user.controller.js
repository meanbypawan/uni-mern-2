...
export const signInAction = async (request,response,next)=>{
  try{
    ...
    return User.checkPassword(password,encryptedPassword) ? response.status(200).json({message: "Sign in success",user}) : response.status(401).json({error: "Unauthorized user"});
  }
  catch(err){
    console.log(err);
    return response.status(500).json({error: "Internal Server Error"});
